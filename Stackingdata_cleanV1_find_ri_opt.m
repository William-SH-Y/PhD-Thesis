...
for iii = ijk:length(farray_all)
    farray = farray_all(1:iii);
...
%     msssaeC = cluster(msssZ, 'Maxclust', 4);
    ...
    clear numC
    n = length(farray);%floor(length(farray)/2)-3 ;%16;%length(farray)-1;
    msSSE = zeros(n, 1);
    for i = 1:n
        numC = i;%i+3;
    ...
    numC_opt(iii-ijk+1) = find(ddmsSSE == min(ddmsSSE)) + 1;
    ...
    for j = 1:length(numC) %with increasing number of clusters
        ...
        if numC(j) == numC_opt(iii-ijk+1)
            ri_opt(iii-ijk+1) = ri(end);
        end
    ...
    clear numC
    n = length(farray)-1;%floor(length(farray)/2) ;%16;%length(farray)-1;
    fsSSE = zeros(n, 1);
    for i = 1:n
        numC = i;
    ...
    numC_opt_fs(iii-ijk+1) = find(ddfsSSE == min(ddfsSSE)) + 1;
    ...
    %ah1 = ceil(sqrt(length(numC)));
    for j = 1:length(numC) %with increasing number of clusters
        ri_fs = zeros(length(farray)-numC(j) , 1);
        for i = numC(j):length(farray) %Given a number of clusters, keep increasing number of simulations and calculate rand index with respect of number of simulations. Check to see if ri converges
            FShighmahaltr_stack = FShighmahaltr(1:i,1:i);
            fsZ_stack = linkage(FShighmahaltr_stack);
            fsbcC_stack = cluster(fsZ_stack, 'Maxclust', numC(j));
            ...
            ri_fs(i-(numC(j)-1)) = rand_index(fsbcC_stack, msbcC_stack);
        end
        if numC(j) == numC_opt_fs(iii-ijk+1)
            ri_opt_fs(iii-ijk+1) = ri_fs(end);
        end
        %subplot(ah1, ah1, j); %subfigure number indicates various number of clusters. In each figure, horizontal axis indicates increasing number of simulations used to calculate ri 
        %plot(ri_fs);
    end
    
    fsbcC_opt = cluster(fsZ, 'Maxclust', numC_opt_fs(iii-ijk+1));
    msbcC_opt = cluster(msbcZ, 'Maxclust', numC_opt(iii-ijk+1));
    ri_opt_fsms(iii-ijk+1) = rand_index(fsbcC_opt, msbcC_opt);
